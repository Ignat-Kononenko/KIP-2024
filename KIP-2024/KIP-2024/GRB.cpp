...
#define TS(n) Rule::Chain::T(n)
	...
	short Rule::getNextChain(GRBALPHABET t, Rule::Chain& pchain, short j) ///Получить след. за j цепочку, вернуть ее номер или -1
	{
		...
		if (rc >= 0) 
			pchain = chains[rc];
		/*	chains[rc]; std::cerr << "Found chain with t: " << Chain::alphabet_to_char(t) << std::endl;
		*/
	
		//else {
		//	std::cerr << "Chain not found for t: " << Chain::alphabet_to_char(t) << std::endl; // Добавлен вывод	
		//}
		return rc;
	};
