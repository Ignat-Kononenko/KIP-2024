...
namespace Polish {
    bool PolishNotation(int i, LexAnal::Lex& lex) {
        ...
        for (int j = i; lex.lextab.table[j].lexema != LEX_SEMICOLON; ++j) {
            
            if (lex.lextab.table[j].lexema == 'v') {
               
                if (strcmp(lex.idtab.table[lex.lextab.table[j].idxTI].id, "+") == 0) {
                    cout << '+' << ' ';
                }
                else if (strcmp(lex.idtab.table[lex.lextab.table[j].idxTI].id, "-") == 0) {
                    cout << '-' << ' ';
                }
                else if (strcmp(lex.idtab.table[lex.lextab.table[j].idxTI].id, "*") == 0) {
                    cout << '*' << ' ';
                ...
                cout << lex.lextab.table[j].lexema << ' ';
            }
        }
        cout << endl;

        for (; lex.lextab.table[i].lexema != LEX_SEMICOLON; i++, lexem_counter++) {
        ...
        while (lexem_counter != 0) {
            if (!queue.empty()) {
                ...
                if (queue.front().lexema == LEX_OPERATOR) {
                    cout << lex.idtab.table[queue.front().idxTI].id << ' ';
                }
                else {
                    cout << queue.front().lexema << ' ';
                }
                queue.pop();
            }
            else {
