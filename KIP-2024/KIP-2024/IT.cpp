...
namespace IT
{
	...
	void ShowTable(IdTable idt, std::ostream& stream_out)
	{
		...
		for (i = 0, j = 0; i < idt.size; i++, j++) {
			...
			if (idt.table[i].iddatatype == INT && (idt.table[i].idtype == V || idt.table[i].idtype == L))
				stream_out << std::setw(18) << std::left << idt.table[i].value.vint;
			/*else if (idt.table[i].iddatatype == UINT && (idt.table[i].idtype == V || idt.table[i].idtype == L))
			...
			else if (idt.table[i].iddatatype == CHAR && (idt.table[i].idtype == V || idt.table[i].idtype == L))
				stream_out << "[" << idt.table[i].value.vchar.lenghtOfChar << "]\"" << idt.table[i].value.vchar.ch << "\"";
			else stream_out << "-";
			stream_out << std::endl;
		}
