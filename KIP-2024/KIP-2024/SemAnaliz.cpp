    ...
    bool Parse(LexAnal::Lex lex, Log::LOG log)
    {
        ...
        for (int i = 0; i < lex.lextab.size; i++)
        {
            ...
            case LEX_ID:
                ...
                if (lex.idtab.table[lex.lextab.table[i].idxTI].idtype == IT::F && lex.lextab.table[i - 1].lexema != LEX_FUNCTION)
                {
                    ...
                    while (lex.lextab.table[j].lexema != LEX_RIGHTHESIS) {
                        if (lex.lextab.table[j].lexema == LEX_ID || lex.lextab.table[j].lexema == 'l')
                        {
                            if (lex.idtab.table[lex.lextab.table[i].idxTI].params.count == 1) {
                                count++;    // 
                            }
            ...
            case LEX_EQUALS:
                ...
                for (j; lex.lextab.table[j].lexema != LEX_SEMICOLON; j++)
                {
                    ...
                    case LEX_LITERAL:
                        ...
                        if ((lex.idtab.table[lex.lextab.table[j].idxTI].iddatatype != lex.idtab.table[lex.lextab.table[i - 1].idxTI].iddatatype))
                        {
                            std::cerr << "Типы данных не совпадают: " << lex.idtab.table[lex.lextab.table[j].idxTI].iddatatype << " != " << lex.idtab.table[lex.lextab.table[i - 1].idxTI].iddatatype << std::endl;
                            answer = false;
                            throw ERROR_THROW_IN(312, lex.lextab.table[j].sn, j);
                        }
                        if (lex.lextab.table[j - 1].op == LT::ODIV && lex.idtab.table[lex.lextab.table[j].idxTI].idtype == IT::L && ((lex.idtab.table[lex.lextab.table[j].idxTI].value.vint == 0)))
                        {
                            answer = false;
            ...
            case LEX_CYCLE:
                ...
                j = i + 2;
                if (lex.idtab.table[lex.lextab.table[j].idxTI].iddatatype != IT::INT)
                {
                    throw ERROR_THROW(311);
                }
                if (lex.idtab.table[lex.lextab.table[j].idxTI].value.vint < 1)
                {
                    throw ERROR_THROW(311);
                }
                break;
